...
namespace AIWordPressManager.Desktop.ViewModels
{
    public sealed partial class MainWindowViewModel
    {
        ...
        private string _scheduledSyncPauseSiteId = string.Empty;
        ...
        public string ScheduledSyncPauseSiteId
        {
            get => _scheduledSyncPauseSiteId;
            private set => SetProperty(ref _scheduledSyncPauseSiteId, value);
        }
        ...
        internal void ApplyScheduledSyncPause(ScheduledSyncPauseInfo? pause)
        {
            if (pause is null || pause.RetryAt <= DateTimeOffset.Now)
            {
                ...
                ScheduledSyncPauseSiteId = string.Empty;
            ...
            ScheduledSyncPauseSiteId = pause.SiteId;
            ...
            ScheduledSyncPauseDetail = $"Site {pause.SiteId} reached {pause.FailureCount} consecutive failures. The scheduler is protecting the site from repeated requests.";
            ...
            JourneyProfessionalReason = $"Automatic retry is scheduled for {pause.RetryAt.LocalDateTime:g}. Review the failed job now or wait for the protected retry window.";
            JourneyProfessionalStatusBrush = Brushes.DarkOrange;
            CurrentJourneyStepTitle = "Review the paused synchronization";
            CurrentJourneyStepDescription = "Inspect the last failure, correct credentials or connectivity, then retry safely from Jobs.";
        ...
        {
            if (!HasScheduledSyncPause || _scheduledSyncPauseRetryAt is null)
                return;
...
{
    internal sealed record ScheduledSyncPauseInfo(string SiteId, int FailureCount, DateTimeOffset RetryAt, DateTimeOffset DetectedAt);

    internal static partial class ScheduledSyncDashboardAlert
    {
        ...
        {
            if (Timer.Tag is not MainWindow window || !window.IsLoaded)
                return;
        ...
        private static void InstallAlert(MainWindow window)
        {
            ...
            if (marker?.Parent is not StackPanel headerPanel)
                return;
        ...
        {
            if (window.DataContext is not ViewModels.MainWindowViewModel viewModel || _alertBorder is null)
                return;
            ...
            var pause = ReadLatestPause(latestLog);
            viewModel.ApplyScheduledSyncPause(pause);
            _alertBorder.Visibility = viewModel.HasScheduledSyncPause ? Visibility.Visible : Visibility.Collapsed;
        }
        ...
        private static ScheduledSyncPauseInfo? ReadLatestPause(string path)
        {
            try
            {
                ...
                for (var index = lines.Count - 1; index >= 0; index--)
                {
                    ...
                    var failureCount = int.Parse(match.Groups["failures"].Value, CultureInfo.InvariantCulture);
                    ...
                    if (DateTimeOffset.TryParse(match.Groups["retry"].Value, CultureInfo.CurrentCulture, DateTimeStyles.AssumeLocal, out var parsedRetry))
                        retryAt = parsedRetry;

                    return new ScheduledSyncPauseInfo(match.Groups["site"].Value, failureCount, retryAt, DateTimeOffset.Now);
                }
            }
            catch
            {
                // Dashboard diagnostics must never interrupt the application.
            }
