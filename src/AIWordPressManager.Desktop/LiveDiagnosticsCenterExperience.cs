...
internal static class LiveDiagnosticsCenterExperience
{
    ...
    private static void OnMainWindowLoaded(object sender, RoutedEventArgs e)
    {
        if (sender is not MainWindow window || window.Content is not Grid root ||
            FindNamedElement<Button>(root, "LiveDiagnosticsButton") is not null) return;

        var button = new Button
        {
            Name = "LiveDiagnosticsButton", Content = "◉", ToolTip = "Live Diagnostics Center",
            Width = 42, Height = 38, HorizontalAlignment = HorizontalAlignment.Right,
            VerticalAlignment = VerticalAlignment.Top, Margin = new Thickness(0, 5, 60, 0),
            FontSize = 17, FontWeight = FontWeights.Bold, Cursor = System.Windows.Input.Cursors.Hand
        };
...
internal sealed class LiveDiagnosticsCenterWindow : Window
{
    ...
    private LiveDiagnosticsCenterWindow()
    {
        ...
        root.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        root.Children.Add(new TextBlock { Text = "Live Diagnostics", FontSize = 24,
            FontWeight = FontWeights.Bold, Margin = new Thickness(0, 0, 0, 14) });

        var cards = new UniformGrid { Columns = 3, Margin = new Thickness(0, 0, 0, 16) };
        ...
        cards.Children.Add(CreateCard("Dispatcher", _dispatcher));
        cards.Children.Add(CreateCard("Session uptime", _uptime));
        Grid.SetRow(cards, 1); root.Children.Add(cards);

        _details.IsReadOnly = true; _details.AcceptsReturn = true; _details.TextWrapping = TextWrapping.NoWrap;
        ...
        _details.FontSize = 12; _details.Padding = new Thickness(14);
        Grid.SetRow(_details, 2); root.Children.Add(_details);

        var actions = new StackPanel { Orientation = Orientation.Horizontal,
            HorizontalAlignment = HorizontalAlignment.Right, Margin = new Thickness(0, 14, 0, 0) };
        var refresh = CreateButton("Refresh now"); refresh.Click += (_, _) => RefreshSnapshot();
        var collect = CreateButton("Collect memory"); collect.Margin = new Thickness(8, 0, 0, 0);
        collect.Click += (_, _) =>
        {
        ...
        actions.Children.Add(refresh); actions.Children.Add(collect); actions.Children.Add(copy);
        ...
        Loaded += (_, _) => { RefreshSnapshot(); _timer.Start(); };
    ...
    private void RefreshSnapshot()
    {
        try
        {
            ...
            process.Refresh();
            var openWindows = GetWindows().Count(window => window.IsVisible);
            var timers = CountDispatcherTimers();
            var dispatcherState = Dispatcher.CurrentDispatcher.HasShutdownStarted ? "Stopping" : "Responsive";
            var uptime = _uptimeWatch.Elapsed.ToString(@"hh\:mm\:ss");
            ...
            _dispatcher.Text = dispatcherState;
            _uptime.Text = uptime;
            ...
            _details.Text = string.Join(Environment.NewLine,
                ...
                $"Threads: {process.Threads.Count}", $"Open visible windows: {openWindows}",
                $"Discovered DispatcherTimers: {timers}", $"Dispatcher state: {dispatcherState}",
                $"Session uptime: {uptime}", string.Empty, "Visible windows:",
                string.Join(Environment.NewLine, GetWindows().Where(window => window.IsVisible)
                    .Select(window => $"  - {window.GetType().Name}: {window.Title}")));
        }
        catch (Exception ex) { _details.Text = ex.ToString(); }
    }
    ...
    private static int CountDispatcherTimers()
    {
        ...
        foreach (var window in GetWindows())
        {
            ...
            if (window.DataContext is not null) count += CountTimersInObject(window.DataContext, visited, 0);
        }
        return count;
    }
    ...
    private static int CountTimersInObject(object instance, HashSet<object> visited, int depth)
    {
        ...
        {
            object? value; try { value = field.GetValue(instance); } catch { continue; }
            if (value is DispatcherTimer timer) { if (timer.IsEnabled) count++; continue; }
    ...
    {
        value.FontSize = 21; value.FontWeight = FontWeights.Bold; value.Margin = new Thickness(0, 6, 0, 0);
        var panel = new StackPanel();
        panel.Children.Add(new TextBlock { Text = title, FontSize = 12, Opacity = .68,
            FontWeight = FontWeights.SemiBold });
        panel.Children.Add(value);
        var border = new Border { Margin = new Thickness(4), Padding = new Thickness(16),
            CornerRadius = new CornerRadius(12), BorderThickness = new Thickness(1), Child = panel };
    ...
    {
        var button = new Button { Content = text, MinWidth = 120, Height = 38,
            Padding = new Thickness(14, 8, 14, 8), Cursor = System.Windows.Input.Cursors.Hand };
