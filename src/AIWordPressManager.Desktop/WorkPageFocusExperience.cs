...
internal static class WorkPageFocusExperience
{
    ...
    {
        "Dashboard", "Sites", "Help", "Settings", "Notification Center"
    };

    private static readonly string[] AuxiliaryPanelTags =
    [
        "PriorityResolutionPanel", "ReviewWorkbenchesPanel", "ContentQualityBatchPanel",
        "QuickFixJourneyPanel", "MediaAnalysisPanel", "AiCopilotInboxPanel"
    ];
    ...
    private static Border BuildPrimaryActionBar(State state)
    {
        ...
        state.ActionButton.Click += state.OnActionButtonClick;
    ...
    private static void Apply(State state)
    {
        ...
        foreach (var surface in Enumerate<FrameworkElement>(state.Root))
        {
            ...
            var tag = surface.Tag?.ToString();
            if (tag is "FloatingWorkspaceScrim" or "FloatingWorkspaceLauncher") continue;
    ...
    private static void UpdateWorkspaceSummary(State state)
    {
        ...
        state.WorkspaceSummary.Text =
            $"{site.DisplayHost} • {site.StatusLabel} • Sync: {lastSync} • Pending: {main.SuggestedChanges.PendingCount} • Approved: {main.SuggestedChanges.ApprovedCount} • Ready: {main.ExecutionCenter.ReadyCount} • Running: {main.DashboardRunningJobs} • Errors: {main.DashboardFailedJobs}";
    }
    ...
    private static T? FindByTag<T>(DependencyObject root, string tag) where T : FrameworkElement
    {
        ...
        {
            var result = FindByTag<T>(VisualTreeHelper.GetChild(root, i), tag);
    ...
    private sealed class State(MainWindow window, Grid root, MainWindowViewModel main)
    {
        ...
        public async void OnActionButtonClick(object sender, RoutedEventArgs e)
        {
            try { await ExecuteNextActionAsync(this); }
            catch { }
        }
        ...
        private void OnPropertyChanged(object? sender, PropertyChangedEventArgs e)
        {
            if (e.PropertyName is null or nameof(MainWindowViewModel.CurrentPage) or
                ...
                nameof(MainWindowViewModel.DashboardLastSiteSync) or
                nameof(MainWindowViewModel.DashboardRunningJobs) or
                nameof(MainWindowViewModel.DashboardFailedJobs) or
                "SelectedSite" or "PendingCount" or "ApprovedCount" or "ReadyCount" or
                "ExecutedCount" or "LoadedAt" or "LoadedItemsCount")
                Apply(this);
        }

        private void OnCollectionChanged(object? sender, NotifyCollectionChangedEventArgs e) => Apply(this);
        ...
        private void OnClosed(object? sender, EventArgs e)
        {
            ...
            if (ActionBar.Parent is Panel parent) parent.Children.Remove(ActionBar);
        }
    }

    private readonly record struct NextActionInfo(NextAction Action, string Label, string Description);
    private enum NextAction { None, SelectSite, RetestConnection, Synchronize, GenerateSuggestions, Review, Execute, Verify, RunAudit }
}
